...
| LLM | DeepSeek API（`deepseek-chat` 为主，价值分级可用 `deepseek-reasoner`） | 国内可直连、便宜、候选人已有使用经验 |
| LLM/搜索抽象 | `AnswerProvider` 接口：`DeepSeekProvider`（真实）+ `MockProvider`（fixtures 回放） | 演示可复现；未来接 ChatGPT/Gemini 只需新增实现类 |
...
按性价比排序：
1. [ ] FastAPI 暴露 `POST /diagnose` + 极简 Vue3+TS 前端页（输入品牌 → 展示报告）——呼应简历技术栈
2. [ ] 第二个真实 Provider（如可访问的 Gemini API 或 SERP API），报告呈现跨平台对比
3. [ ] Query Fanout：每个高价值问题自动派生 3~5 个子问法并抽样检测
4. [ ] 5 分钟演示视频/GIF
...
- 模式判定：有 `DEEPSEEK_API_KEY` → hybrid（LLM 真实、AI 平台回答可 mock）；无 → 全 mock。
