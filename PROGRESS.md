...
## 2026-07-17 session-11 (Codex，评审成品化精简与全局动效)

### 用户反馈复盘
- [x] 确认上一版仍把“向项目所有者解释迭代”的内容放进了“给评审者看的成品”：顶部线路说明、
      URL 未回填状态、技术页国内/国际部署解释均不属于 HR 或技术评审的核心阅读任务。
- [x] 建立前端内容准入规则：只保留帮助评审理解产品、检查证据、判断工程能力和打开交付物的
      内容；部署选择、环境状态、修改记录和未完成事项全部退出前端，留在 README、SUBMISSION
      与 deploy 文档。

### 成品化精简
- [x] 删除顶部线路状态和下拉说明，右侧改为唯一有用的“完整报告”入口；导航只承担受众切换。
- [x] 技术页删除国内/国际、网络切换、URL 和公网状态，第四部分改为“稳定回放 / 受控实况”，
      直接展示可复现、密钥隔离、限流、并发与超时的工程思路。
- [x] 清理“这里不重复产品故事”“不再把你送到”等描述页面设计过程的措辞，改为直接说明
      评审能看到什么、如何核验。
- [x] 产品视角完成第二轮术语翻译：Fanout → 派生问法，Mock → 离线模式，Pydantic → 统一
      数据契约，Web Search → 联网搜索；前三条建议去除错误代码，改为业务行动标题。
- [x] 删除不再使用的 `VITE_DOMESTIC_URL` / `VITE_INTERNATIONAL_URL` 前端配置；双公网地址只在
      `SUBMISSION.md` 交付说明中维护。

### 全局视觉与动效
- [x] 视角切换增加淡出 / 位移 / 模糊恢复过渡，顶部双按钮使用滑动底板，产品和技术切换保持
      空间连续性。
- [x] Hero 文案与数据板分层进入，背景加入低对比度环形漂移动效；卡片、建议、架构模块、
      诊断节点均增加克制的 hover / active 反馈。
- [x] 新增 IntersectionObserver 滚动渐显：每个主章节进入视口后只播放一次；实测触发后最终
      opacity=1、transform=none，不会把内容永久隐藏。
- [x] `prefers-reduced-motion` 下关闭动画和过渡，内容直接可见，保证可访问性。

### 受众可理解性复核
- [x] 产品路径：产品定义 → 核心结论与三指标 → 测试题六项回答 → 七步诊断 → 三项行动 →
      完整报告；页面未发现国内/国际/线路/回填等项目管理文案。
- [x] 技术路径：工程目标与交付状态 → 七模块契约 → 按 run_id 追踪 → 可靠性边界 → 受控执行 →
      三步复现；产品页 DOM 在切换后为 0，信息体系完全独立。
- [x] 1280×900 与 390×844 均无横向溢出；顶部、首屏、报告入口和双视角切换可直接定位；
      浏览器控制台零 error / warning。

### 最终闸门
- [x] Vue typecheck、国内/国际 `build:all`、55 项 pytest、Mock 20 / 8 / 7 / 7、部署脚本语法
      与 `git diff --check` 全部通过；国际静态包仍无 LivePanel chunk。
- [x] 最终 Lighthouse：LCP 1793.251ms、TBT 43.238ms、CLS 0、5 请求、约 95KiB；主 JS
      37.19KB gzip、CSS 6.58KB gzip，全部在既定预算内。

### 决策记录
- 展示页不承担部署说明书职责。即使某项技术事实正确，只要它不帮助评审判断候选人的业务理解、
  拆解、交付或工程能力，就不进入前端。
- 动效服务于层级、因果和可点击反馈，不展示动画本身；全部动效只作用于 opacity、transform、
  filter，并提供 reduced-motion 降级。

### 给 Fable5 的复核重点
1. 重点搜索前端是否仍残留线路、部署、URL 回填或迭代日志式文案；
...
## 2026-07-17 session-14 (Codex，公网实况入口与候选人标识)

### 评审入口
- [x] 吸顶导航新增蓝色“现场演示”按钮，产品与技术视角都会平滑定位到各自的
      `#live-demo`；完整报告入口仍保留在同一导航区。
- [x] 导航品牌信息与两套 Hero 均加入“刘畅 · AI 研发工程师候选作品”，不把个人信息
      塞进业务结论卡，保持评审记忆点与报告叙事分层。

### 实况可读性与停止语义
- [x] 实况说明、护栏标签、按钮、状态栏、回答与错误文字整体放大；错误/停止态终端
      从 500px 空白面板收紧到 260~290px，同时保留运行态的三题滚动空间。
- [x] 前端停止后先显示“正在安全停止”，900ms 内不允许立即重启；完成后明确提示
      可以再次开始，减少用户撞上 409 并发保护。
- [x] 后端等待 DeepSeek 子进程输出时每 0.5 秒检查浏览器是否断开；发现断开即终止
      子进程并在 `finally` 释放全局锁，不再等待整个 180 秒超时。
- [x] 新增断开检测回归测试；当前总数 `57 passed`，Mock 仍为 `20 / 8 / 7 / 7`，
      国内/国际双构建和 Vue TypeScript 检查通过。

### 决策记录
- 409 并发限制仍保留，因为它防止重复点击并发消耗 API；本次修复的是客户端主动停止后
  服务器锁释放不够及时，不是删除保护。

---

## 2026-07-17 session-15 (Codex，实况锁泄漏根因修复)

### 根因证据
- [x] 用户在公网再次收到 409 时，服务器 `demo_worker=none`、FastAPI 仍为 active；证明不是
...
- [x] 端点返回响应前只做 `semaphore.locked()` 快速判断，不占用锁；占锁移入
      `event_stream` 内部，并由同一个 `try/finally` 负责 worker 终止与锁释放。
- [x] 新增“返回流响应后、首次迭代前立即关闭”回归测试，直接断言此时信号量
...
### 候选人与现场入口
- [x] Hero 候选人标识改为独立高对比卡；“刘畅”使用 23px 粗体，岗位使用 12px 说明，
      同时保留导航内的姓名识别。
- [x] 吸顶按钮改为橙色高对比“现场试一下～”，增加呼吸状态点；点击后仍精确定位
      产品/技术各自的现场模块。
...
## 2026-07-17 session-20 (Codex，用户人工验收与交付收口)

### 公网验收口径
- [x] 用户使用正常浏览器完成公网网页与报告检查，并反馈“都没啥问题”；按用户决定停止继续压测，进入交付。
- [x] 本记录只声明用户人工验收通过，不冒充 Codex 自动化浏览器取证；10 次混合压力操作未形成独立自动化证据。
- [x] LiveLease 自动化回归仍由 5 个新增测试覆盖；服务端部署后健康接口已确认 `live_busy=false`。

### 一致性收口
- [x] 当前面向评审的 README、SUBMISSION、方案说明、部署文档和双视角网页统一为 **63 passed**。
- [x] 应聘岗位沿用用户明确给出的“AI 研发工程师”，候选人为“刘畅”。
- [x] 两条固定入口统一为国内动态版 `http://101.201.79.59:8080/` 与国际静态版
      `https://ai-market-diagnostic.pages.dev/`；国际版更新以重新上传 `web/dist-intl/` 为准。
- [x] `npm run build:all` 重新生成国内 `web/dist/` 与国际 `web/dist-intl/`；国际包无
      `LivePanel`、`live-diagnose` 或 `/api/` 引用，首页与 `/report/` 静态文件齐全。
- [x] 最终冻结回归：`63 passed`；Mock 基线保持
      `20 问题 / 8 回答 / 7 缺口 / 7 建议`。
