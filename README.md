...
**报告中的数字能直接用于客户决策吗？** 不能。提交示例是 DeepSeek 的单轮 8 问真实调用，不等价于 ChatGPT/Gemini 市场表现。来源 URL 确实来自 Web Search 结果，但未逐句人工核对每个来源与结论的支持关系。商业测量仍需接入真实海外平台，扩到 100+ 问题并做多轮采样与证据抽检。
...
当前测试集共 17 个用例，覆盖启发式回答解析、品牌/竞品顺位、引用与情感识别、Web Search 响应解析与 URL 传递、指标聚合口径、SQLite 报告往返，以及 Deli 消歧不污染通用问题、竞品名称规范化等真实链路回归场景。
