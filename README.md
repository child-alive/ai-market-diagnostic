...
默认回放模式读取固化的真实运行 `37b442ec`，零 API 消耗。顶部吸顶导航切换两套独立表达：**产品视角**逐项回应测试题六项考察并用业务语言解释结论，**技术视角**展示模块、数据契约、原始 JSON、来源与等价命令。产品与技术视角都能进入受控实况；只有评审者显式点击后才运行固定的 3 个高价值无品牌词。实况限制为每 IP 每小时 2 次、全局并发 1、总超时 180 秒，Key 只存在服务端。未配置 DeepSeek 时实况返回友好提示，回放和完整报告仍可用。

仓库同时保留国内动态与国际静态两套部署产物；两者页面内容和 `/report/` 相同，仅实况 API 能力不同。双地址放在提交说明中，不占用评审页面的信息空间。
...
**“来源覆盖率 100%”是否等于“品牌被引用”或“来源支持结论”？** 都不等于。Citation Rate（来源覆盖率）只统计平台 API 是否返回搜索来源，与目标品牌是否出现在回答或来源中无关。加 `--verify-evidence` 后，系统会抓取来源页面并保存最匹配证据片段与支持状态，但这仍是词面机器预审；报告会固定标注“需人工复核”。
