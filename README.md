...
ECharts 通过 CDN 加载；断网时竞品排行表与其余全部内容仍可正常阅读。提交示例位于 `data/示例产物/`，其中 AI 回答为 Mock，官网结果为 2026-07-16 对 `deliworld.com` 的 15 页实时检查。
...
**报告中的数字能直接用于客户决策吗？** 不能。提交示例只有 8 条 Mock AI 回答，适合验证产品链路与结论表达；商业测量需接入真实海外平台、扩到 100+ 问题并做多轮采样。
...
当前测试集共 12 个用例，覆盖启发式回答解析、品牌/竞品顺位、引用与情感识别、指标聚合口径和 SQLite 报告往返。
