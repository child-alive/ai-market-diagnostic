...
默认回放模式读取固化的真实运行 `37b442ec`，零 API 消耗。顶部吸顶导航切换两套独立表达：**产品视角**逐项回应测试题六项考察并用业务语言解释结论，**技术视角**展示模块、数据契约、原始 JSON、来源与等价命令。实况入口只在技术视角出现，且只有评审者显式点击后才运行固定的 3~5 个高价值无品牌词；它限制为每 IP 每小时 2 次、全局并发 1、总超时 180 秒，Key 只存在服务端。未配置 DeepSeek 时实况返回友好提示，回放和完整报告仍可用。

国内动态版与国际静态版不是按访问者网络自动切换，而是同一前端的两个明确部署地址：内容和 `/report/` 相同，仅实况 API 能力不同。页面右上角的“线路说明”会标明当前能力；只有回填了另一条真实公网 URL 才显示切换链接，避免把本机端口或空地址展示给评审者。
