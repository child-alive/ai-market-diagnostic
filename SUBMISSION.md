...
4. **可选：双受众演示网页**
   ...
   - 国内动态版（回放 + 显式实况）：`待按 deploy/部署手册.md 部署 ECS 后回填`
   - 国际静态版（回放 + 报告，无 API）：`待按 deploy/Cloudflare Pages部署指引.md 发布后回填`

   演示采用国内外双线路部署，与本诊断服务的出海场景一致。

   <!-- TODO(提交前处理): 部署后回填两个 URL；若最终不部署，删除整个第 4 步。 -->
   无需 Node.js 的本地预览：`.venv/bin/uvicorn src.demo_api:app --host 127.0.0.1 --port 8765`，
   然后访问 `http://127.0.0.1:8765/`。国内版默认回放零额度、实况必须显式点击；国际版在
   构建时关闭实况。两个版本的 `/report/` 都是固化完整报告。当前的 `report.html` 仍是独立
   完整的提交主件。
...
- 完成 Vue 3 + TypeScript 双受众网页：真实数据七阶段回放、产品/技术视角、完整报告入口，以及固定 Prompt Set、SSE 流式进度、限流/并发/超时保护的 FastAPI 实况模式；共 55 个回归测试。
...
3. 用户按部署手册将已验收网页发布到 ECS，并将 IP:端口回填本页；
